...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer** We are creating a new dataframe called "baltco_911_calls_by_month" by modifying the dataframe "baltco_911_calls." We are creating a new column called "month," which is take the date listed in the "date" column, identifying it's month, and then adding that value as an abbreviation (instead of a number) to that new column. We then take the data and put it into group by month and identify the total numbers of calls per month. Lastly, we organize the data based on the number of calls is descending order. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** A blank gray box.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I see a vertical bar chart with count as the y axis and month as the x axis. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It is the same bar char, but this time the months are listed starting with the month with the lowet count, then moving to the highest count. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The axis have flipped, so the bars are now running horizontally, with the months on the y axis and count on the x axis. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** Instead of having a light gray background with white lines, the background is now white with light gray lines. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** The full background color is now a blueish-gray. There are no vertical lines in the background, only white horizontal lines that run through (and behind) the middle of the bars. Also, the labels and number line are now black instead of gray. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The chart has been given a title, "More 911 Overdose Calls in Warmer Months," which is also bold. Also, the y axis is now labeled "month" and the x axis is now labeled "total calls." A line about the source of the data was also added. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I see a tibble listing the total calls for each date in the dataset. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** I see a chart that looks like a sound wave. Total calls is on the y axis and the data is on the x axis. The chart shows how many call occurred each day and connects those points chronologically with a line. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** There is now more labels on the x axis, which make the it very hard to reach each one. The line chart itself seems to be the same. The labels look to be the only thing that changed. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The labels for the x axis are now slated at a 45 degree angle so that we can reach each one. 

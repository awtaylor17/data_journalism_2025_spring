...
**Answer** 79366 obs. and 9 variables
...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer** The data type for "id" is chr. The data type for "seats" is dbl.
...
**Answer** "umd_courses |>" is letting RStudio know that we want to work with data from the "umd_courses" dataset. "group_by(term) |>" is saying that we want to sort the data into piles based by based on the "term" variable. "summarise(count_classes = n())" is telling RStudio we want to make the those piles from the "count_classes" variable. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** The order of this codeblock has changed, compared to the previous codeblock. It went from being ordered by the "term" variable to being ordered by the "count_classes" variable in descending order. It did not change the data we received, but it did change the order of how we received it. 
...
**Answer** In this codeblock, instead of seeing the total classes for each term, we see how many classes each department offered each term. The code that changed it was adding the "department" variable to the group_by function. The arrange function was also changed, which affected how the data was presented to us. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** This time, the data is only being grouped by the "term" variable, however, in addition to displaying the number of classes in each term, RStudio is also displaying the total number of seats (total_seats) in each term. The change that was made is that "total_seats = sum(seats)" was added to the summarise function. That code adds the sum of all the seats in each term to the output under the heading "total_seats." The arrange function was also changed to display the data in descending order based on total_seats. 
...
**Answer** In this code block, we added two more variables to the summarise function, those being the mean number of seats and the median number of seats. In the output, we now have columns that show us the mean and median number of seats for each term. The top terms all have a mean number of seats around 47, and a median number of seats around 24.
...
**Answer** Unlike the last couple codeblocks, this codeblock is grouped by the department, not by the term. That means that all the values in the columns created by the summarise function have changed because they are being sorted by a different variable. It also added the minimum and maximum number of seats in each department.
...
1.  How many courses in the classes with the highest seat count are general education courses? How about courses that are prerequisites for other departments?

2.  How many students of each year are in each course?

3.  Do departments with higher seat counts get more money from the university?

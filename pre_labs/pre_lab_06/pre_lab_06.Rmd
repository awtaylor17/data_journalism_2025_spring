...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** They do have the same column structure, but, in some, the order of the unaffilated the other columns are switched. 
...
**Task** Load the Maryland 2020 county population data **Answer** This dataframe has two columns, one with the county name and another with the county's population in 2020. 
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24 rows, and that's the number that there should be because maryland_population (left dataset and the one that we are basing the join off of) has 24 rows. 
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer** It tells us that the function is joining the two datasets by the COUNTY column. 
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** Prince George's County has the highest percentage of Black people over 18 and the highest percentage of Democrats. 
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Howard County has the highest population of Asian people over 18, with Montgomery County not too far behind. Those two counties are significantly higher than the rest of the counties because, which has an Asian 18+ population that is in the single digits. 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **Answer** The ZCTA5N column is a dbl (so, a number).
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** It is now a character. 

...
**Task** Run the following code and describe the dataframe it outputs. **Answer** The dataframe 499 different instances of a disciplinary alerts. It contains information on the name and type of the doctor and when the alert was made. It also contains a column with the text of the alert. 
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** The function lists the number of unique words in the list, but it counts differences in capitalization as being unique, as well as symbols, like the comma.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** The output of the `unnest_tokens` function is a dataframe with one column (called "word") where each row is a word of the text. 
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** The graphic is a horizontal bar chart that shows the words that have the highest unique count. To me, it is not interesting because the top words are those that are used in every sentence like "the" and "and." Those words aren't nessicarily unqiue to this text and don't give us much insight into the alerts. 
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** This codeblock gives us a table with the unique words, how many times they are observed and their percentage of the total. This result is much more interesting because the words relate more to the alerts. 
...
**Task** Run the following code and describe the results. **Answer** The two tables list the top words for each year and their count and percentage of the total. In 2023, "license," "disciplinary" and "health" were the top three words, while in 2022, the top three words were "disciplinary," "health" and "patient."
...
**Task** Run the following code and describe the results. **Answer** This code outputs a table that lists the bigram, how many times it is observed and what percentage it makes up. 
...
**Task** Run the following code and describe the results. **Answer** This code gives us the same result as the previous codeblock but for 2023. The results look to be the same, at least for the top 10 results. 
...
**Task** Run the following code and describe the results. **Answer** This codeblock gives us the bigram count for after 2024. "Health occ" is still the tob bigram, but "summary suspension" and "individual 4" moved to the second and third place spots. 
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** This code gives us two dataframes (one for 2022 and one for 2023) that list the unique words, whether it has a positive or negative connotation, and their counts. There are some works that are given a connotation that may not fit it. For example, "substantive" is given a positive connotation, but, if it was talking abount a "substantive mistake," that could be a very bad connotation. 

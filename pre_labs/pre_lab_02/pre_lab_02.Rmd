...
**Answer** It is important to create a new variable because we don't want to permanently change the original dataset, in case we make a mistake or want to use it for other analysis. 
...
**Answer** 54000, so more than the first filter. 
...
**Answer:** The dates in the expenditure_date column go from being written in m-d-y format to being written in y-m-d format. Additionally, the data type changes from being a character to being identified as a date.
...
**Answer** The values in the month column are all in y-m-d format with the day for every month being 1. I think that R is grouping the total_amount for each month under the row for the first of the month, then displaying those in descending order based on the total_amount. 
...
**Answer** This code is creating two new columns in the data frame general_22. The first is called total_votes and it adds together the columns for cox, moore, lashar, wallace and the write_ins. The second is pct_moore, which is taking the moore column and dividing it by the newly created total votes column. 
...
**Answer** The answer is now being displayed in descending order based off of the pct_moore column. So the counties that had the higher percentage of moore voters are displayed first. We changed this by using the arrange() and desc() functions. 
...
**Answer** I don't think it is necessarily a more interesting answer because the counties that had a small percentage of people that voted for moore are just as interesting and noteworthy as those counties that had a large percentage of moore voters. Both are important to know, so one is not more useful than the other, in my opinion. 
...
**Answer** Two function that I found are str_to_lower() and str_to_sentence(). str_to_lower() makes everything lowercase while str_to_sentence() makes it so only the first letter in every sentence is capitalized. 
...
**Answer** The code is telling R to look at the address column and when it detects the name of the state listed below, it should add the listed abbreviation of that state to the new "state" column. If it doesn't detect any of the listed states, it should put NA in the "state" column.
...
**Answer** Seeing that Maryland has the highest total does make sense, since this data is about campaign expenses for Maryland politicians. However, I think including NA in the rundown is misleading because the NA value can be from any state. The only reason they got grouped together is because their address didn't mention anything about one of the states we were looking for.
...
1.  Which committee spent the most each year? (filtering and dates)

2. Which committee had the highest percentage of expenses? (mutate)

3.  How much money did Facebook receive from Maryland campaign committees? (filtering)

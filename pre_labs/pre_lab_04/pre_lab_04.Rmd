...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** One example is the Walter P. Carter Child Center. It is listen once as "WALTER P CARTER CHILD CARE CENTER," then again as "WALTER P CARTER CHILD CARE CENTER INC." Even though they are the same company, the computer groups then differently because they have a slightly different name. 
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer** One change is that the Washington County Museum of Fine Arts, capitalized into: WASHINGTON COUNTY MUSEUM OF FINE ARTS. Another is that the Charles County Arts Alliance Inc was truncated and capitlized into: CHARLES COUNTY ARTS ALLIANCE.
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** 1 - It took away the "INC" from WALTER P CARTER CHILD CARE CENTER INC. 2 - WASHINGTON ADVENTIST UNIVERSITY was lowercased (I feel like this is an interesting change, since, in the last couple examples, the function capitalized the grantee.) I was not able to find a third change. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** The changes don't look to be that different from the previous method. Truthfully, I'm not able to spot a difference between the two methods. 

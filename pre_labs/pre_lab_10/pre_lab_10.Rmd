...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** I see eight columns that contain each row's object ID, zip code (in two different formats), where it exists (which I'm guessing is when it was made) shape length, shape area and geometry, and the name of the zip code/area. 
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** It contains geographic information for a number of zip codes in Maryland. The lengths and areas of these zip codes seem to be pretty large, as all of the values are greater than 2,000 in length and 300,000 in area. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It looks like these zip codes make up the entire state of Maryland. The map shown below looks like an outline of the entire state, with no noticeable gaps, at least from what I can see. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** When I installed the package, it asked me to restart R, which I did. When I loaded the library, it gave me a warning saying this package was built in an earlier version of R, and it let me know that I can enable caching of the data. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This dataset has an ID for the state and for the county. It includes the names of the counties: one where the "County" is added at the end of the name and one where it is not. I also includes a variety of different geographic data  for each county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code created a new dataframe with just the Maryland counties (the ones where the "STATEFP" is equal to 24)
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code created a map of Maryland, like before, but this time it only had the county outlines and not the zip codes. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This time, the map contains both the county outlines and the zip codes. They seem to be same same color and thickness, though, so it is hard for me to tell when one county ends and another begins. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code created a dataframe that contains a zip code and how many foreclosures that zip code had in March 2023. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code added the number of forclosures in March 2023 for each zip code to the dataframe with the geographic data. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code creates a choropleth map that colors each zip code based on the number of foreclosures it had. The lighter colors had more foreclosures and the darker colors had less. 
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer** The code loaded the packages and gave me a warning that tidycensus was built in a different version of R. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code created two dataframes. The first contains the number of foreclosure notices in each Maryland County for March of 2023. The second has the total population for each Maryland county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code added the geometry column to the dataframe with the Maryland county population. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code loaded the two dataframes. We can see that the column that contains the county name data is different. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The county population datafrme is now cleaned. The "name" column is now called "county" and just lists the county name with no ", Maryland."
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code joined the two dataframes. So we have geographic data, population data and the number of notices all in one dataframe. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code creates a column with the rate of foreclosures per 1000 people and orders is in descending order. Charles County has the most forclosures per 1000 people. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code create a choropleth map with percapita rate of foreclosures. The lighter colors have more foreclosures and the darker colors have the least. The map also has labels for each county. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code made the differences in the per capita rates more noticeable. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** This code created a more nuanced map with more variations for each county. It shows us that Charles County has the most number of foreclosures, and that counties like Baltimore City, Prince George's County and Cecil County have the second highest amount of foreclosures. One pattern that this map shows me is that the number of foreclosures isn't based on how urban the county is. This is because both urban and rural counties have high and low foreclosure numbers on the map.

...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** I see that the inputs under the "maryland" column have periods at the end of them. Additionally, every column name except for "maryland" is a number and not a descriptive name. 
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer** Both problems still remain, but this time the column names are in numberical order instead of random numbers. 
...
**Task** Run the codeblock below. What else do we need to do? **Answer** We still need to remove the periods from the jurisdiction column. 

...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** It does not show any parsing errors this time. 
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer** 553 zip codes do not have five characters. I think that's because there is either an error with the data or the zip codes are representing a group of zip codes (e.g. a general area) instead of one specific code. 
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer** The first line is saying that we want to use the "md_grants_loans" dataset. The second is grouping putting the data into groups based on their fiscal year and category value. The third is saying that we want to see how many observations are in each group. The last line is arranging our data in ascending order based on their fiscal year. What stands out to me as being problematic is that there are a few categories that are have an "NA" value. Also, one of the categories only says "L" instead of loan, I'm guessing. 
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer** The main thing that stands out to me is that most of the data are grants and not loans, which somewhat surprises me. Another thing is that some of the grantor and grantee names are title case and some are all capitalized (and some have a combination of both.) Also, some zip codes are in ranges, instead of just one code. 
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer** It makes the names of the columns lower cased and adds an underscore in place of a space. 
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer** The first line is saying that we want to put the output into a new data frame. It is also saying that we want to use the "md_grants_loans" dataset. The second row is saying that we want to use the clean_names() function to standardize the column names. The fourth line is saying that we want to rename the column "grantor" to "source" The fifth line is saying that we want to make the source, grantee and description columns upper case. The last line is saying that we want to see the output of the new dataframe we created in the first line. 
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** There are 58 individual roles that have duplicates and about 19 different sets of duplicated values. 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** A new column was created called "zip5." In it, the zip code values from the data set have been truncated to only include 5 characters.
